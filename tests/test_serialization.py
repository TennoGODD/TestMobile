...
class TestSerialization:
    ...
    def test_serialization_with_aggregation_level_0(self, serialization_page, db, task, printer_kigu):
        ...
        with shared_step(f"Сканируем КМ №1"):
        ...
        for i, code in enumerate(codes[1:-1], start=2):
            with shared_step(f"Сканируем КМ №{i}"):
                serialization_page.emulate_scan(code)
            serialization_page.verify_serialization_data(
        ...
        last_code = codes[-1]
        with shared_step(f"Сканируем КМ №6: {last_code}"):
        ...
        with shared_step(f"Сканируем код агрегата"):
    ...
    def test_serialization_with_aggregation_levels_0_1(self, serialization_page, db, task, printer_kigu, printer_kitu):
        ...
        for i in range(1, 5): 
            code = codes[i]
            with shared_step(f"Сканируем КМ №{i + 1}"):
                serialization_page.emulate_scan(code)
                serialization_page.verify_serialization_data(
        ...
        with shared_step(f"Сканируем код №6"):
        ...
        with shared_step(f"Сканируем агрегат 0 уровня №1"):
            for attempt in range(1, 3):
                with shared_step(f"Сканируем агрегат 0 уровня №1 (попытка {attempt}): {unit_id_0_first}"):
        ...
        with shared_step(f"Сканируем агрегат 0 уровня №2"):
            for attempt in range(1, 3):
                with shared_step(f"Сканируем агрегат 0 уровня №2 (попытка {attempt}): {unit_id_0_second}"):
